 ...
 * @author Shivani Singh, ss@g.rit.edu
 */
public class BeeHive {
    ...
    /**
     * number of nectar bees
     */
    private int numNectarBees;
    /**
     * number of pollen bees
     */
    private int numPollenBees;
    ...
    public BeeHive(int numDrones, int numNectarWorkers, int numPollenWorkers) {
        ...
        numNectarBees = numNectarWorkers;
        numPollenBees = numPollenWorkers;

        // create the Queen
        this.bees.add(Bee.createBee(Role.QUEEN, Resource.NONE, this));

        //create the drone bees
    ...
    public synchronized boolean hasResources() {
        return getRemainingNectar()>0 && getRemainingPollen()>0;
    }
    ...
    public synchronized void deposit(Resource resource, Worker bee) {
        ...
        if(resource.name().equalsIgnoreCase("NECTAR")){
            nectarGathered++;
            nectar++;
        }
        else if (resource.name().equalsIgnoreCase("POLLEN")){
            pollenGathered++;
            pollen++;
        }
        notify();
    ...
    /**
     * To get the number of nectar bees
     * @return int
     */
    public int getNumNectarBees() {
        return numNectarBees;
    }

    /**
     * Getter method to get the number of pollen bees
     * @return int
     */
    public int getNumPollenBees() {
        return numPollenBees;
    }
}
