...
public abstract class Bee extends Thread {
    ...
    public static Bee createBee(Role role, Worker.Resource resource, BeeHive beeHive) {
        ...
        if (role.name().equalsIgnoreCase("QUEEN")){
            System.out.println("*BH* QUEEN #"+ID+" is born!");
            return new Queen(beeHive);
        }
        else if (role.name().equalsIgnoreCase("DRONE")){
            System.out.println("*BH* DRONE #"+ID+" is born!");
            return new Drone(beeHive);
        }
        System.out.println("*BH* "+resource.name()+" WORKER #"+ID+" is born!");
